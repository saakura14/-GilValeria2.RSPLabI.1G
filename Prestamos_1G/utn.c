...
int utn_getNombre(char* pResultado, int longitud,char* mensaje, char* mensajeError, int reintentos)
{
	...
	while(reintentos>=0)
	{
		reintentos--;
		printf("%s",mensaje);
		if(getNombre(bufferString,sizeof(bufferString)) == 0 && my_strnlen(bufferString,sizeof(bufferString)) < longitud )
		{
...
int utn_getCuil(char* pResultado, int longitud,char* mensaje, char* mensajeError, int reintentos)
{
	...
	while(reintentos>=0)
	{
		reintentos--;
		printf("%s",mensaje);
		if(getCuil(bufferString,sizeof(bufferString)) == 0 && my_strnlen(bufferString,sizeof(bufferString)) < longitud )
		{
			strncpy(pResultado,bufferString,longitud);
			retorno = 0;
			break;
